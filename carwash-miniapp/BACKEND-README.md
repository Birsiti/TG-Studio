...
Создать новую Google Таблицу, в ней — 12 вкладок **с точно такими названиями и заголовками колонок в первой строке** (регистр важен):
...
### ВЫПЛАТЫ
```
id | staff_id | amount | date | created_at
```
Заполняется автоматически при нажатии «Выплатить» в панели владельца — вкладку
нужно создать заранее (с заголовками), сами строки добавлять не нужно.

### ТЕМЫ
```
tg_id | theme | updated_at
```
Личная цветовая схема каждого пользователя (client/admin/owner — общий лист
на все три роли), привязана к его tg_id. Заполняется автоматически при выборе
схемы через кнопку «Цветовая схема» — вкладку нужно создать заранее (с
заголовками), сами строки добавлять не нужно.
