...
```
Старт
  └─ Условие (Да/Нет)                  — что именно проверяет, не выяснено, см. «Открытые вопросы»
...
Что конкретно проверяет блок «Условие» (Да/Нет) сразу после «Старт», до
приветствия — не выяснено (не первый ли раз пользователь? тег? что-то
другое). Уточнить у Дениса при следующей возможности и дополнить сюда.
